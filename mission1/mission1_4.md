### ❓ 문제 4 - 서비스 가용성 확보를 위한 임시 파일 자동 정리 문제

### 가정 상황

- 운영 중인 서버의 디스크 사용량이 90%를 넘어 서비스 중단(Disk Full) 위기에 처했습니다. 인프라 관리자는 어느 디렉토리에 용량이 큰 파일이 몰려 있는지 **계층적으로 파악**하고, 임시 파일들이 차지하는 **전체 용량을 계산**하여 보고한 뒤, 이를 **주기적으로 정리**하는 자동화 스크립트를 배포해야 합니다.
