...
### 5-4.
  > 최종 필터링된 결과를 `awk`를 사용하여 아래의 규격화된 포맷으로 변환하고, `secret_scan_report.txt` 파일로 저장하시오.<br>
...
```
#### [명령어 설명]
##### 에러 메시지를 숨기는 용으로 권한이 없어서 못 읽는 파일이 있을 때 또는 Permission denied 에러로를 모아서 휴지통(/dev/null)으로 버립니다. 결과 화면을 깨끗하게 유지해주기 위함입니다.
```
2>/dev/null
```
